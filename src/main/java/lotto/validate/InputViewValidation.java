...
public class InputViewValidation {
    ...
    public static void checkLottoBonus(String input, List<Integer> lottoAnswer) {
        if (onlyNumber(input)) {
            if (checkOneBetween1And45(input) && checkLottoAnswerNotContainsString(input,
                lottoAnswer)) {
                return;
            }
        }
    ...
    private static boolean checkLottoAnswerNotContainsString(String input,
        List<Integer> lottoAnswer) {
